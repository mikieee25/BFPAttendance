...
def capture_attendance_enhanced():
    ...
    Accepts either a single main_image or a frames array (last frame is used).

    Request JSON format:
    {
        "main_image": "base64_image",          // primary image
        "frames": ["base64_image1", ...]       // optional; last frame used if main_image absent
    }
    """
    try:
        data = request.get_json()
        ...
        # Process the image and extract face
        face_embedding, face_metadata, temp_path = process_base64_image(main_image_data)
        ...
        logger.error(f"Error in attendance capture: {e}")
        return handle_api_exception(e)
